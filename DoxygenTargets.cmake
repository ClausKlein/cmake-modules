...
function(
  ...
  if(CMAKE_VER VERSION_LESS 2.8.0)
    ...
    install(FILES "${filename}" DESTINATION "${dest}" RENAME "${rename}" ${COMPONENT_ARGS} OPTIONAL)
  endif()
...
function(_dt_install_dir target dir dest)
  if(CMAKE_VER VERSION_LESS 2.8.0)
    ...
    install(DIRECTORY "${dir}" DESTINATION "${dest}" ${COMPONENT_ARGS} OPTIONAL)
  endif()
...
function(add_doxygen _doxyfile)
  ...
  set(_bool_args NO_WARNINGS NO_PDF)
  ...
  if(DOXYGEN_FOUND)
    if(NOT TARGET ${DOC_TARGET})
      ...
      if(NOT IN_DASHBOARD_SCRIPT)
        ...
        set_target_properties(${DOC_TARGET} PROPERTIES EXCLUDE_FROM_ALL TRUE)
        set_target_properties(${DOC_TARGET} PROPERTIES EXCLUDE_FROM_DEFAULT_BUILD TRUE)
      else()
    ...
    if(NOT IS_ABSOLUTE "${OUTPUT_DIRECTORY}")
      get_filename_component(OUTPUT_DIRECTORY "${CMAKE_CURRENT_BINARY_DIR}/${OUTPUT_DIRECTORY}" ABSOLUTE)
    endif()

    set_property(
      DIRECTORY APPEND PROPERTY ADDITIONAL_MAKE_CLEAN_FILES "${OUTPUT_DIRECTORY}/html" "${OUTPUT_DIRECTORY}/latex"
    )
    ...
    configure_file(
      "${_doxygenmoddir}/DoxygenTargets.doxyfile.in" "${CMAKE_CURRENT_BINARY_DIR}/${_doxyfile}.additional" @ONLY
    )
    ...
    if(MAKE_PDF)
      add_custom_command(
        ...
        COMMENT "Generating PDF using PDFLaTeX..."
        VERBATIM
      )
    endif()
    ...
    if(INSTALL_DESTINATION)
      if(INSTALL_COMPONENT)
        ...
        if(MAKE_PDF)
          _dt_install_file(
            "${DOC_TARGET}"
            "${OUTPUT_DIRECTORY}/latex/refman.pdf"
            "${INSTALL_DESTINATION}"
            "${INSTALL_PDF_NAME}"
            "${INSTALL_COMPONENT}"
          )
        ...
        if(MAKE_PDF)
          _dt_install_file(
            "${DOC_TARGET}" "${OUTPUT_DIRECTORY}/latex/refman.pdf" "${INSTALL_DESTINATION}" "${INSTALL_PDF_NAME}"
          )
        endif()
      endif()
    endif()
